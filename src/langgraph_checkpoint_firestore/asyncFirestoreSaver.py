...
class AsyncFirestoreSaver(BaseCheckpointSaver):
    ...
    ) -> Iterator["AsyncFirestoreSaver"]:
        saver = None
        try:
            saver = AsyncFirestoreSaver(
    ...
    async def aput(
        ...
        await self.checkpoints_collection.document(thread_id).collection(
            checkpoint_id
        ).document("data").set(data)
        return {
            "configurable": {
                "thread_id": thread_id,
                "checkpoint_ns": checkpoint_ns,
    ...
    async def aput_writes(
        ...
        for idx, (channel, value) in enumerate(writes):
            ...
            await self.writes_collection.document(thread_id).collection(
                checkpoint_id
            ).document(task_id).set(data)
    ...
    async def _load_pending_writes(
        ...
        writes_ref = self.writes_collection.document(thread_id).collection(
            checkpoint_id
        )  # Use collection instead of collections()
        matching_keys = [write_doc.to_dict() async for write_doc in writes_ref.stream()]
        parsed_keys = [
    ...
    async def _get_checkpoint_key(
        ...
        async for collection in collections:
            doc = await collection.document("data").get()
            if doc.exists:
